...
export const getFichaF1_2doAno = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM ficha_f1_2do_ano_2026 WHERE estudiante_id = $1::uuid LIMIT 1`,
      [estudiante_id]
    );
    ...
    console.error("Error GET Ficha F1 2do Año:", error);
    return res.status(500).json({ message: "Error al consultar la base de datos.", error: error.message });
  }
};

// POST - SAVE OR UPDATE (MAPEO ROBUSTO DE CRITERIOS)
export const saveOrUpdateFichaF1_2doAno = async (req, res) => {
  ...
  try {
    ...
    // Mapeo flexible: Busca tanto 'f1_criterio_X' como posibles nombres del objeto de configuración
    const crit1 = Math.min(100, Math.max(0, parseFloat(datos.f1_criterio_1 ?? datos.criterio_1 ?? datos.criterio1) || 0));
    const crit2 = Math.min(100, Math.max(0, parseFloat(datos.f1_criterio_2 ?? datos.criterio_2 ?? datos.criterio2) || 0));
    const crit3 = Math.min(100, Math.max(0, parseFloat(datos.f1_criterio_3 ?? datos.criterio_3 ?? datos.criterio3) || 0));
    const crit4 = Math.min(100, Math.max(0, parseFloat(datos.f1_criterio_4 ?? datos.criterio_4 ?? datos.criterio4) || 0));
    ...
    const docId = (datos.docente_iepc_id && String(datos.docente_iepc_id).trim() !== '') ? datos.docente_iepc_id : null;
    ...
    const checkRes = await client.query(
      `SELECT id FROM ficha_f1_2do_ano_2026 WHERE estudiante_id = $1::uuid LIMIT 1`,
      [estudiante_id]
    );
    ...
    if (checkRes.rowCount > 0) {
      ...
      values = [
        ...
        obs, docId, nombres, esfm, esp, ciudad, dep,
        dia, mes, ano, est, estudiante_id
      ];
    } else {
      query = `
        INSERT INTO ficha_f1_2do_ano_2026 (
          estudiante_id, f1_criterio_1, f1_criterio_2, f1_criterio_3, f1_criterio_4,
          promedio_numeral, promedio_literal, observaciones, docente_iepc_id,
    ...
    console.error("Error SAVE Ficha F1 2do Año:", error);
...
export const deleteFichaF1_2doAno = async (req, res) => {
  const { estudiante_id } = req.params;
  try {
    const result = await pool.query(
      `DELETE FROM ficha_f1_2do_ano_2026 WHERE estudiante_id = $1::uuid RETURNING id`,
      [estudiante_id]
    );
    ...
    console.error("Error DELETE Ficha F1 2do Año:", error);
