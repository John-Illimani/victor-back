...
// GET - OBTENER FICHA B-3 5TO AÑO
export const getFichaB3_5toAno = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM ficha_b3_5to_ano_2026 WHERE estudiante_id = $1::uuid LIMIT 1`,
      [estudiante_id]
    );
    ...
    console.error("Error GET Ficha B-3 5to Año:", error);
...
export const saveOrUpdateFichaB3_5toAno = async (req, res) => {
  ...
  try {
    ...
    const query = `
      INSERT INTO ficha_b3_5to_ano_2026 (
        estudiante_id, apellidos_nombres, nota_c1, nota_c2, nota_c3,
        puntaje_final, promedio_numeral, promedio_literal, logros_dificultades,
        lugar_ciudad, departamento, dia, mes, ano, estado
      ) VALUES (
        $1::uuid, $2, $3, $4, $5, $6, $6, $7, $8, $9, $10, $11, $12, $13, $14
      )
      ON CONFLICT (estudiante_id) DO UPDATE SET
        ...
        logros_dificultades = EXCLUDED.logros_dificultades,
    ...
    const values = [
      estudiante_id, apellidos_nombres, c1, c2, c3,
      prom, promedio_literal, logros_dificultades,
    ...
    return res.status(200).json({ message: "Ficha B-3 de 5to Año guardada exitosamente.", datos: result.rows[0] });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error("Error SAVE Ficha B-3 5to Año:", error);
...
export const deleteFichaB3_5toAno = async (req, res) => {
  const { estudiante_id } = req.params;
  try {
    const result = await pool.query(
      `DELETE FROM ficha_b3_5to_ano_2026 WHERE estudiante_id = $1::uuid RETURNING id`,
      [estudiante_id]
    );
    ...
    console.error("Error DELETE Ficha B-3 5to Año:", error);
