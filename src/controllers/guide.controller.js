...
// 1. OBTENER TODOS LOS DOCENTES GUÍA
export const getGuides = async (req, res) => {
  try {
    ...
    return res.status(500).json({ 
      message: "Error al obtener la lista de docentes guía.", 
      error: error.message 
    });
  }
};

// 2. CREAR UN DOCENTE GUÍA INDIVIDUAL
export const createGuide = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({
      message: "Docente Guía registrado exitosamente.",
      guide: result.rows[0]
    });
  } catch (error) {
    console.error("Error al registrar docente guía:", error);
    return res.status(500).json({ message: "Error al registrar docente guía.", error: error.message });
  }
};
...
export const updateGuide = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      message: "Docente guía actualizado correctamente.",
      guide: result.rows[0]
    });
  } catch (error) {
...
// 5. ELIMINACIÓN MASIVA DE DOCENTES GUÍA
export const deleteMultipleGuides = async (req, res) => {
  const { ids } = req.body;

  if (!ids || !Array.isArray(ids) || ids.length === 0) {
    return res.status(400).json({ message: "Debe proporcionar un arreglo de IDs válido." });
  }
  ...
  try {
    ...
    return res.json({
      message: `Se eliminaron ${result.rowCount} docentes guía correctamente.`,
      count: result.rowCount
    });
  } catch (error) {
    console.error("Error en eliminación masiva de docentes guía:", error);
    return res.status(500).json({ message: "Error al eliminar la lista de docentes guía.", error: error.message });
  }
};
...
export const importBatchGuides = async (req, res) => {
  ...
  if (!docentes || !Array.isArray(docentes) || docentes.length === 0) {
    return res.status(400).json({ message: "Se requiere un arreglo de docentes guía válido." });
  }
  ...
  try {
    ...
    return res.json({
      message: `Se importaron/actualizaron ${insertados} docentes guía exitosamente.`,
      count: insertados
    });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error("Error al importar docentes guía:", error);
