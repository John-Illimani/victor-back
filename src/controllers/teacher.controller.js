...
// 1. OBTENER DOCENTES CON CONTEO Y SUS FORMULARIOS HABILITADOS
export const getTeachers = async (req, res) => {
  try {
    const query = `
      ...
      WHERE d.rol = 'DOCENTE_ACOMPANANTE'
      GROUP BY d.id
    ...
    return res.status(500).json({ message: "Error al consultar docentes.", error: error.message });
  }
};

// 2. CREAR DOCENTE ACOMPAÑANTE
export const createTeacher = async (req, res) => {
  ...
  try {
    ...
    const values = [
      ...
      esfm_ua || "ESFM/UA - El Alto", especialidad || null, item_docente || null
    ];

    const result = await pool.query(query, values);
    return res.status(201).json({ message: "Docente registrado con éxito.", teacher: result.rows[0] });
  } catch (error) {
...
export const updateTeacher = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(query, values);
    if (result.rowCount === 0) return res.status(404).json({ message: "Docente no encontrado." });

    return res.json({ message: "Docente actualizado correctamente.", teacher: result.rows[0] });
  } catch (error) {
...
export const deleteTeacher = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: "Error al eliminar docente.", error: error.message });
  }
};
...
export const deleteMultipleTeachers = async (req, res) => {
  ...
  try {
    ...
    return res.json({ message: `Se eliminaron ${result.rowCount} docentes exitosamente.`, count: result.rowCount });
  } catch (error) {
    console.error("Error en eliminación masiva:", error);
    return res.status(500).json({ message: "Error al eliminar la lista de docentes.", error: error.message });
  }
};
  ...
  try {
    ...
    for (const doc of docentes) {
      ...
      const values = [
        ...
        doc.esfm_ua || "ESFM/UA - El Alto", doc.especialidad || null, doc.item_docente || doc.codigo || null
      ];
    ...
    return res.json({ message: `Se registraron/actualizaron ${insertados} docentes exitosamente.`, count: insertados });
  } catch (error) {
...
// 7. OBTENER ESTUDIANTES ASIGNADOS A UN DOCENTE
...
// 8. ASIGNAR ESTUDIANTE A UN DOCENTE
export const assignStudentToTeacher = async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(400).json({ message: "El estudiante ya se encuentra asignado a este docente." });
    }
...
// 9. QUITAR ASIGNACIÓN DE ESTUDIANTE
export const unassignStudentFromTeacher = async (req, res) => {
  const { docente_id, estudiante_id } = req.body;
  try {
    const query = `
...
// 10. TOGGLE HABILITAR/DESHABILITAR FORMULARIO DIRECTO AL DOCENTE
export const toggleFormularioDocente = async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE id = $3::uuid AND rol = 'DOCENTE_ACOMPANANTE'
    ...
    return res.json({ message: "Permiso de formulario actualizado correctamente.", docente: result.rows[0] });
  } catch (error) {
    console.error("Error al actualizar formulario del docente:", error);
